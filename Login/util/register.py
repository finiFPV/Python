...
def email_verify(email):
    ...
    verif1 = str(input(f'{Fore.CYAN}Enter the verification code sent to your email. >>'))
    clear()
    if result != verif1:
        email_valid = False
    ...
    with open('util\\database.txt', 'r') as reg:
        for line in reg:
            if email_save in line:
                ...
                return
    clear()
    bitcoin_address = input(f'{Fore.CYAN}Enter your bitcoin address. Leave empty if you dont have one. >>')
    clear()
    global bitcoin_save
...
#to find database enteries somthing.{something}.{user_name}.xyzdt28@~